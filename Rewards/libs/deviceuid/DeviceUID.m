...
//  Library
//
//  Created by Anthonio Ez on 29/08/2016.
//  Copyright © 2016 Frenzin Softwares. All rights reserved.
//
...
- (void)save: (NSString*) id
{
    if (![DeviceUID valueForUserDefaultsKey:_uidKey]) {
        [DeviceUID setValue:id forUserDefaultsKey:_uidKey];
    }
    if (![DeviceUID valueForKeychainKey:_uidKey service:_uidKey]) {
        [DeviceUID setValue:id forKeychainKey:_uidKey inService:_uidKey];
    }
}
...
+ (BOOL)isDebugMode
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        if (data) {
            ...
            isDebugMode = [cleared rangeOfString:@"<key>get-task-allow</key><true/>"].length > 0;
        }
    });
    return isDebugMode;
}
