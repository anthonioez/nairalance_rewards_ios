...
{
    static let buttonTapped = #selector(ItemPicker.buttonTapped)
    static let deviceOrientationDidChange = #selector(ItemPicker.deviceOrientationDidChange)
}

open class ItemPicker: UIView, UIPickerViewDelegate, UIPickerViewDataSource
{
    public typealias ItemPickerCallback = ( Int ) -> Void

    private var selectedIndex = -1;
    private var pickerList = [[String: Any]]()
    ...
    // MARK: - Variables
    private var callback: ItemPickerCallback?
    ...
    open func show(_ title: String,
                   ...
                   callback: @escaping ItemPickerCallback)
    {
        self.pickerList.removeAll();
        self.pickerList.append(contentsOf: list);
    ...
    private func createContainerView() -> UIView {
        ...
        self.itemPicker = configuredItemPicker()
        container.addSubview(self.itemPicker)
    ...
    fileprivate func configuredItemPicker() -> UIPickerView
    {
        let datePicker = UIPickerView(frame: CGRect(x: 5, y: 30, width: 0, height: 0))
        datePicker.setValue(self.textColor, forKeyPath: "textColor")
        datePicker.autoresizingMask = .flexibleRightMargin
        datePicker.frame.size.width = 290
    ...
    {
        let item = pickerList[row];
        let titleData = item["title"] as? String;
        ...
        let attributedTitle = NSAttributedString(string: titleData!, attributes: [NSAttributedStringKey.font: Rewards.font, NSAttributedStringKey.foregroundColor:UIColor.black])
